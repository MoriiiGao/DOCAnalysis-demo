...
def feature_visualization(features, model_type, model_id, feature_num=36):
    ...
    blocks = torch.chunk(features, feature_num, dim=1)
    plt.figure()
    for i in range(2):
        b = blocks[i]
        torch.squeeze(b)
        b = blocks[i].squeeze()
        feature = transforms.ToPILImage()(b)
        # print(feature)
    ...
    # plt.imshow(features.cpu())

    # torch.squeeze(features)
    # feature = transforms.PILToTensor()(features.squeeze())
    # ax = plt.subplot(int(math.sqrt(feature_num)), int(math.sqrt(feature_num)), i + 1)
    # ax.set_xticks([])
    # ax.set_yticks([])
    # plt.imshow(feature)
...
def show_feature_map(feature_map, outRoot):  # feature_map=torch.Size([1, 64, 55, 55]),feature_map[0].shape=torch.Size([64, 55, 55])
    ...
    for index in range(1, feature_map_num + 1):  # 通过遍历的方式，将64个通道的tensor拿出
        feature_mask = feature_map[index - 1].cpu().numpy()
        ...
        # plt.axis('off')
        cv2.imwrite(f'{outRoot}//' + str(index) + ".png", feature_mask)
...
#             f = str(save_dir) + "/" + "stage{}_{}_features.png".format(stage, module_type.split('.')[-1])  # filename
#
